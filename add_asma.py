...
def main():
    print("Fetching songs for Asma Lmnawar...")
    # Trying different spellings for iTunes indexing
    results = search_itunes("Asma Lmnawar")
    results.extend(search_itunes("Asmaa Lamnawar"))
    results.extend(search_itunes("Asma Lmanawar"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Asma' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Asma' not in artist and 'أسماء' not in artist and 'اسماء' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'أسماء لمنور'
        ...
        song = {
            ...
            "artist": artist,
            "title": title,
            "artistAr": "أسماء لمنور",
            "titleAr": title_ar,
            "year": year,
            "styles": [genre] if genre else ["Khaliji", "Moroccan Pop"],
            "link": preview_url,
            "artworkUrl": artwork
        }
    ...
    with open('asma_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to asma_new.json.")
