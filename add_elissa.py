...
def main():
    print("Fetching songs for Elissa...")
    results = search_itunes("Elissa")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Elissa' in s.get('artist', '') or 'اليسا' in s.get('artist', '') or 'إليسا' in s.get('artist', '')}
    ...
    for r in results:
        ...
        # Exact match or close match for Elissa to avoid noise
        if artist != 'Elissa' and artist != 'اليسا' and artist != 'إليسا':
            if 'Elissa' not in artist and 'اليسا' not in artist and 'إليسا' not in artist:
                continue
        ...
        if not artist_ar: artist_ar = 'إليسا'
    ...
    with open('elissa_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to elissa_new.json.")
