...
def main():
    print("Fetching songs for Mohamed Ramadan...")
    results = search_itunes("Mohamed Ramadan")
    results.extend(search_itunes("محمد رمضان"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Ramadan' in s.get('artist', '') or 'رمضان' in s.get('artistAr', '')}
    
    new_songs = []
    
    base_id = "ara_mramadan"
    ...
    for r in results:
        ...
        # Check if artist is roughly Mohamed Ramadan or محمد رمضان
        artist_lower = artist.lower()
        if 'mohamed' not in artist_lower and 'mohammad' not in artist_lower and 'محمد' not in artist:
            continue
        if 'ramadan' not in artist_lower and 'رمضان' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'محمد رمضان'
        ...
        song = {
            ...
            "titleAr": title_ar,
            "year": year,
            "styles": [genre] if genre else ["ArabicPop", "Egyptian", "Rap"],
            "link": preview_url,
            "artworkUrl": artwork
        }
