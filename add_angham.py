...
def main():
    print("Fetching songs for Angham...")
    results = search_itunes("Angham")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Angham' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Angham' not in artist and 'أنغام' not in artist and 'انغام' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'أنغام'
    ...
    with open('angham_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to angham_new.json.")
