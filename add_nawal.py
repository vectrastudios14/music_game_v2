...
def main():
    print("Fetching songs for Nawal El Zoghbi...")
    results = search_itunes("Nawal El Zoghbi")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Nawal' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Nawal' not in artist and 'نوال' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'نوال الزغبي'
    ...
    with open('nawal_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to nawal_new.json.")
