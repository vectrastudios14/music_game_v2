...
def main():
    print("Fetching songs for Amal Maher...")
    results = search_itunes("Amal Maher")
    results.extend(search_itunes("امال ماهر"))
    results.extend(search_itunes("آمال ماهر"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Amal' in s.get('artist', '') or 'امال' in s.get('artistAr', '') or 'آمال' in s.get('artistAr', '')}
    ...
    for r in results:
        ...
        # Check if artist is roughly Amal Maher or امال ماهر
        if 'amal' not in artist.lower() and 'امال' not in artist and 'آمال' not in artist:
            continue
        if 'maher' not in artist.lower() and 'ماهر' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'امال ماهر'
