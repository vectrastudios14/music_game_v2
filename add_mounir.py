...
def main():
    print("Fetching songs for Mohamed Mounir...")
    results = search_itunes("Mohamed Mounir")
    results.extend(search_itunes("Mohamed Mounier"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Mounir' in s.get('artist', '') or 'Mounier' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Mounir' not in artist and 'Mounier' not in artist and 'منير' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'محمد منير'
        ...
        song = {
            ...
            "titleAr": title_ar,
            "year": year,
            "styles": [genre] if genre else ["ArabicPop", "Egyptian", "Nubian"],
            "link": preview_url,
            "artworkUrl": artwork
        }
    ...
    with open('mounir_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to mounir_new.json.")
