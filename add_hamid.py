...
def main():
    print("Fetching songs for Hamid El Shaeri...")
    results = search_itunes("Hamid El Shaeri")
    results.extend(search_itunes("Hamid El shaeri"))
    results.extend(search_itunes("حميد الشاعري"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Hamid' in s.get('artist', '') or 'حميد' in s.get('artist', '')}
    
    new_songs = []
    
    base_id = "ara_heshaeri"
    count = 1
    album_counts = {}
    ...
    for r in results:
        ...
        if 'Hamid' not in artist and 'حميد' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'حميد الشاعري'
    ...
    with open('hamid_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to hamid_new.json.")
