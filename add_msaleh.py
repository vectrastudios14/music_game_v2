...
def main():
    print("Fetching songs for Medhat Saleh...")
    results = search_itunes("Medhat Saleh")
    results.extend(search_itunes("مدحت صالح"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Medhat' in s.get('artist', '') or 'مدحت' in s.get('artistAr', '')}
    ...
    for r in results:
        ...
        # Check if artist is roughly Medhat Saleh or مدحت صالح
        if 'medhat' not in artist.lower() and 'مدحت' not in artist:
            continue
        if 'saleh' not in artist.lower() and 'صالح' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'مدحت صالح'
