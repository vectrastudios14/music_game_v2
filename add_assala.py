...
def search_itunes(term, limit=100):
    url = f"https://itunes.apple.com/search?term={urllib.parse.quote(term)}&entity=song&limit={limit}&country=sa"
...
def get_arabic_metadata(track_id):
    url = f"https://itunes.apple.com/lookup?id={track_id}&lang=ar_sa&country=sa"
    req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
    try:
        with urllib.request.urlopen(req) as response:
...
def main():
    print("Fetching songs for Assala Nasri...")
    results = search_itunes("Assala Nasri")
    results.extend(search_itunes("Assala"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Assala' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Assala' not in artist and 'أصالة' not in artist and 'اصاله' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'أصالة'
    ...
    with open('assala_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to assala_new.json.")
