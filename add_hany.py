...
def main():
    print("Fetching songs for Hany Shaker...")
    results = search_itunes("Hany Shaker")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Hany' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Hany' not in artist and 'Hani' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'هاني شاكر'
    ...
    with open('hany_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to hany_new.json.")
