...
def main():
    print("Fetching songs for Samira Said...")
    results = search_itunes("Samira Said")
    results.extend(search_itunes("Samira Saeed"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Samira' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Samira' not in artist and 'سميرة' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'سميرة سعيد'
    ...
    with open('samira_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to samira_new.json.")
