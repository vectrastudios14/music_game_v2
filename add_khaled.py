...
def main():
    print("Fetching songs for Cheb Khaled...")
    results = search_itunes("Cheb Khaled")
    results.extend(search_itunes("Khaled")) # He's often just listed as Khaled
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Khaled' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Khaled' not in artist and 'خالد' not in artist:
            continue
            
        # Avoid DJ Khaled and other Khaleds (mostly focus on rai tracks / Cheb Khaled)
        if 'DJ Khaled' in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'الشاب خالد'
        ...
        song = {
            ...
            "titleAr": title_ar,
            "year": year,
            "styles": [genre] if genre else ["Rai", "Arabic"],
            "link": preview_url,
            "artworkUrl": artwork
        }
    ...
    with open('khaled_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to khaled_new.json.")
