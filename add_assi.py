...
def main():
    print("Fetching songs for Assi El Hallani...")
    results = search_itunes("Assi El Hallani")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Assi' in s.get('artist', '')}
    
    new_songs = []
    
    base_id = "ara_ahallani"
    count = 1
    ...
    for r in results:
        ...
        if 'Assi' not in artist and 'Hallani' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'عاصي الحلاني'
    ...
    with open('assi_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to assi_new.json.")
