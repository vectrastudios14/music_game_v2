...
def main():
    print("Fetching songs for Diana Haddad...")
    results = search_itunes("Diana Haddad")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Diana' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Diana' not in artist and 'ديانا' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'ديانا حداد'
    ...
    with open('diana_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to diana_new.json.")
