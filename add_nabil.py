...
def main():
    print("Fetching songs for Nabil Shuail...")
    results = search_itunes("Nabil Shuail")
    results.extend(search_itunes("Nabeel Shuail"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Nabil' in s.get('artist', '') or 'Nabeel' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Nabil' not in artist and 'Nabeel' not in artist and 'نبيل' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'نبيل شعيل'
    ...
    with open('nabil_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to nabil_new.json.")
