...
def main():
    print("Fetching songs for Carole Samaha...")
    results = search_itunes("Carole Samaha")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Carole' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Carole' not in artist and 'كارول' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'كارول سماحة'
        ...
        song = {
            ...
            "artist": artist,
            "title": title,
            "artistAr": "كارول سماحة",
            "titleAr": title_ar,
            "year": year,
            "styles": [genre] if genre else ["Arabic Pop", "Lebanese"],
            "link": preview_url,
            "artworkUrl": artwork
        }
    ...
    with open('carole_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to carole_new.json.")
