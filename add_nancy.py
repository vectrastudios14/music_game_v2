...
def main():
    print("Fetching songs for Nancy Ajram...")
    results = search_itunes("Nancy Ajram")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Nancy' in s.get('artist', '') or 'نانسي' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Nancy' not in artist and 'نانسي' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'نانسي عجرم'
    ...
    with open('nancy_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to nancy_new.json.")
