...
def search_itunes(term, limit=50):
    url = f"https://itunes.apple.com/search?term={urllib.parse.quote(term)}&entity=song&limit={limit}&country=lb"
...
def get_arabic_metadata(track_id):
    url = f"https://itunes.apple.com/lookup?id={track_id}&lang=ar_sa&country=lb"
    req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
    try:
        with urllib.request.urlopen(req) as response:
...
def main():
    print("Fetching songs for George Wassouf...")
    results = search_itunes("George Wassouf")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if s.get('artist') == 'George Wassouf'}
    
    new_songs = []
    
    # Base ID for George Wassouf custom songs
    base_id = "ara_gwassouf"
    count = 1
    ...
    for r in results:
        ...
        if not artist_ar: artist_ar = 'جورج وسوف'
    ...
    with open('wassouf_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to wassouf_new.json. Check the file then append to songs_arabic.json.")
