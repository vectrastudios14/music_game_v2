...
def parse_songs(text):
    # Regex variations for different block types
    # Case 1: 4 lines (Title, Artist, Album/Year, Duration)
    # Case 2: 3 lines (Title, Artist, Duration)
    # Case 3: "Title - Artist | Year" formats
    ...
    for block in blocks:
        ...
        # Heuristic: First line is usually Title
        # Second line is usually Artist
        ...
        # Clean artist from " Tunes Arabia l تيونز أرابيا" type labels
        artist = artist.replace("Tunes Arabia l تيونز أرابيا", "").strip()
        
        # Sometimes title contains "Artist - Title" or similar
        if " - " in title and artist == "":
            parts = title.split(" - ")
            artist = parts[0].strip()
            title = parts[1].strip()
        
        # Remove junk from title like (Official Lyric Video) or | 2017
