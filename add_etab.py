...
def main():
    print("Fetching songs for Etab...")
    results = search_itunes("Etaab")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Etaab' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Etaab' not in artist and 'عتاب' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'عتاب'
    ...
    with open('etab_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to etab_new.json.")
