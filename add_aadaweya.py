...
def main():
    print("Fetching songs for Ahmed Adaweyah (احمد عدوية)...")
    results = search_itunes("Ahmed Adaweyah")
    results.extend(search_itunes("Ahmed Adaweya"))
    results.extend(search_itunes("احمد عدوية"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Adawey' in s.get('artist', '') or 'عدوية' in s.get('artistAr', '')}
    
    new_songs = []
    
    base_id = "ara_aadaweya"
    # Find the max existing count to avoid id collision if script run multiple times
    existing_ids = [int(s['id'].split('_')[2]) for s in existing_songs if s['id'].startswith(f"{base_id}_")]
    count = max(existing_ids) + 1 if existing_ids else 1
    ...
    for r in results:
        ...
        # Check if artist is roughly Ahmed Adaweyah or احمد عدوية
        artist_lower = artist.lower()
        if 'ahmed' not in artist_lower and 'احمد' not in artist and 'أحمد' not in artist:
            continue
        if 'adawey' not in artist_lower and 'adawy' not in artist_lower and 'عدوية' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'احمد عدوية'
        ...
        song = {
            ...
            "titleAr": title_ar,
            "year": year,
            "styles": [genre] if genre else ["ArabicPop", "Egyptian", "Shaabi"],
            "link": preview_url,
            "artworkUrl": artwork
        }
    ...
    if new_songs:
        ...
        print("Saved directly to assets/songs_arabic.json.")
    else:
