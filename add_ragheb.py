...
def main():
    print("Fetching songs for Ragheb Alama...")
    results = search_itunes("Ragheb Alama")
    results.extend(search_itunes("راغب علامة"))
    results.extend(search_itunes("راغب علامه"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Ragheb' in s.get('artist', '') or 'راغب' in s.get('artistAr', '')}
    ...
    for r in results:
        ...
        # Check if artist is Ragheb Alama
        if 'ragheb' not in artist.lower() and 'راغب' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'راغب علامه'
    ...
    with open('ragheb_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to ragheb_new.json.")
