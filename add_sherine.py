...
def main():
    print("Fetching songs for Sherine...")
    results = search_itunes("Sherine")
    results.extend(search_itunes("Sherine Ahmed"))
    results.extend(search_itunes("Sherine Abdel Wahab"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Sherine' in s.get('artist', '') or 'شيرين' in s.get('artistAr', '')}
    ...
    for r in results:
        ...
        if 'Sherine' not in artist and 'شيرين' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'شيرين عبدالوهاب'
    ...
    with open('sherine_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to sherine_new.json.")
