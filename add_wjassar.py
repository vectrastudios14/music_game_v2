...
def main():
    print("Fetching songs for Wael Jassar...")
    results = search_itunes("Wael Jassar")
    results.extend(search_itunes("وائل جسار"))
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Wael' in s.get('artist', '') or 'وائل' in s.get('artistAr', '')}
    ...
    for r in results:
        ...
        # Check if artist is roughly Wael Jassar or وائل جسار
        if 'wael' not in artist.lower() and 'وائل' not in artist:
            continue
        if 'jassar' not in artist.lower() and 'جسار' not in artist.lower() and 'gassar' not in artist.lower():
            continue
        ...
        if not artist_ar: artist_ar = 'وائل جسار'
        ...
        song = {
            ...
            "titleAr": title_ar,
            "year": year,
            "styles": [genre] if genre else ["ArabicPop", "Lebanese"],
            "link": preview_url,
            "artworkUrl": artwork
        }
