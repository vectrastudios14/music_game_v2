...
def main():
    print("Fetching songs for Hatem Al Iraqi...")
    results = search_itunes("Hatem Al Iraqi")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Hatem' in s.get('artist', '')}
    
    new_songs = []
    
    base_id = "ara_haliraqi"
    count = 1
    ...
    for r in results:
        ...
        if 'Hatem' not in artist and 'حاتم' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'حاتم العراقي'
    ...
    with open('hatem_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to hatem_new.json.")
