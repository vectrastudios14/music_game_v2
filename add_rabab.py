...
def main():
    print("Fetching songs for Rabab...")
    results = search_itunes("Rabab")
    
    with open('assets/songs_arabic.json', 'r', encoding='utf-8') as f:
        existing_songs = json.load(f)
        
    existing_titles = {s['title'].lower() for s in existing_songs if 'Rabab' in s.get('artist', '')}
    ...
    for r in results:
        ...
        if 'Rabab' not in artist and 'رباب' not in artist:
            continue
        ...
        if not artist_ar: artist_ar = 'رباب'
        ...
        song = {
            ...
            "titleAr": title_ar,
            "year": year,
            "styles": [genre] if genre else ["Khaliji"],
            "link": preview_url,
            "artworkUrl": artwork
        }
    ...
    with open('rabab_new.json', 'w', encoding='utf-8') as f:
        json.dump(new_songs, f, indent=4, ensure_ascii=False)
        
    print("Saved to rabab_new.json.")
